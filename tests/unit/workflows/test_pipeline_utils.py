...
def test_evaluate_gates_for_stage_success() -> None:
    ...
    with patch("crewai_book.workflows.pipeline_utils.run_all_gates", return_value={"gate1": True}):
        with patch("crewai_book.workflows.pipeline_utils.QUALITY_GATES", []):
...
def test_evaluate_gates_for_stage_abort() -> None:
    ...
    with patch("crewai_book.workflows.pipeline_utils.run_all_gates", return_value={"gate1": False}):
        with patch("crewai_book.workflows.pipeline_utils.QUALITY_GATES", [gate_mock]):
            res = _evaluate_gates_for_stage(state, PipelineStage.RESEARCH)
            assert res is False
...
def test_render_latex_callback(mock_inject, mock_env, tmp_path: Path) -> None:
    ...
    with patch("crewai_book.latex.post_processor.post_process_latex", return_value="Final TeX"):
        with patch("crewai_book.tools.nb_latex_extractor.extract_notebook_to_latex"):
...
def test_render_latex_callback_exception(tmp_path: Path) -> None:
    ...
    with patch("crewai_book.latex.post_processor.post_process_latex", side_effect=Exception("Mock Error")):
        with pytest.raises(RuntimeError, match="Hard stop: LaTeX render callback failed: Mock Error"):
            _render_latex_callback(output, out_path, state)
