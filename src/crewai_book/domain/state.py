...
class Citation(BaseModel):
    ...
    abstract: str | None = Field(None, description="Abstract or summary of the source")
    confidence_score: float = Field(1.0, ge=0.0, le=1.0, description="Confidence score against hallucinations")
...
class PipelineState(BaseModel):
    ...
    current_stage: str = Field("initialization", description="Current stage of the pipeline")
    topic: str = Field(..., description="Main topic being generated")
    run_id: str = Field(..., description="Unique run identifier")
    errors: list[dict[str, Any]] = Field(default_factory=list, description="Log of non-fatal errors")
    retries_used: int = Field(0, description="Total number of retries used across all agents")
    quality_gates_passed: list[str] = Field(default_factory=list, description="Gates successfully passed")
